...
import { BriefcaseBusiness, CalendarDays, Clock3, Filter, Mail, Phone, Plus, Save, Search, UserCog, X } from 'lucide-react';
import { TenantApi, ApiError } from '../../services/api.js';
import type { Department, Employee, EmployeeOrigin, EmployeeStatus, EmployeeTimelineEvent, JobPosition, TenantUser } from '../../types.js';
...
export const ModuleHR: React.FC = () => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError('');
      const [emps, deps, pos, members] = await Promise.all([
        TenantApi.getEmployees(), TenantApi.getDepartments(), TenantApi.getPositions(), TenantApi.getUsers()
      ]);
      setEmployees(emps);
      setDepartments(deps);
      setPositions(pos);
      ...
      setError(err instanceof ApiError ? err.message : 'Nao foi possivel carregar colaboradores.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-5">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">Base mestre de colaboradores, vinculos e historico do ciclo de vida.</p>
        </div>
        {canCreate && (
      ...
      {selected && <EmployeeDrawer employee={selected} canEdit={canEdit} departments={departments} positions={positions} users={users} onClose={() => setSelectedId(null)} onChanged={(e) => { setEmployees(list => list.map(item => item.id === e.id ? e : item)); }} />}
      {creating && <EmployeeDrawer employee={null} canEdit departments={departments} positions={positions} users={users} onClose={() => setCreating(false)} onChanged={(e) => { setEmployees(list => [e, ...list]); setCreating(false); setSelectedId(e.id); }} />}
    </div>
  );
};
...
const EmployeeDrawer: React.FC<{
  ...
  useEffect(() => {
    ...
    if (employee) TenantApi.getEmployeeTimeline(employee.id).then(setTimeline).catch(() => setTimeline([]));
    else setTimeline([]);
  }, [employee?.id]);
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Nao foi possivel salvar o colaborador.');
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/50 backdrop-blur-xs" {...backdrop}>
      <div className="w-full max-w-2xl h-full bg-white shadow-2xl overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-slate-200 px-5 py-4 flex items-center justify-between z-10">
          <div><h3 className="text-sm font-bold text-slate-900">{employee ? employee.name : 'Novo colaborador'}</h3><p className="text-xs text-slate-500">Ficha operacional, sem dados sensiveis no v1.</p></div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500"><X className="w-4 h-4" /></button>
        </div>
        <form onSubmit={save} className="p-5 space-y-5 text-xs">
