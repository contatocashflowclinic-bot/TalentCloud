...
import type { RetentionPerson } from '../../types.js';
import { useBackdropClose } from '../../hooks/useBackdropClose.js';

interface Props {
  /** null while loading. */
  people: RetentionPerson[] | null;
  /** A person from the list, or null to register someone who is not in it. */
  onPick: (person: RetentionPerson | null) => void;
  onClose: () => void;
}

const ORIGIN_LABEL: Record<RetentionPerson['origin'], string> = {
  pdi: 'Com PDI',
  hire: 'Contratação',
  member: 'Usuário da organização'
};

/** First step of "Registrar Sinal de Risco": pick who the alert is about, so it is tied to a real person. */
export const RetentionPersonPickerModal: React.FC<Props> = ({ people, onPick, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs" {...backdrop}>
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[92vh] flex flex-col border border-slate-200 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="p-5 border-b border-slate-100 flex items-start justify-between gap-3">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-600">Novo alerta</span>
            <h3 className="text-base font-bold text-slate-900">Sobre quem é o sinal de risco?</h3>
        ...
        <div className="p-5 space-y-3 text-xs overflow-y-auto min-h-0">
          ...
          {people === null ? (
            ...
            <p className="text-slate-500 py-4 text-center">
              {people.length === 0 ? 'Todas as pessoas conhecidas já têm um alerta ativo.' : 'Ninguém encontrado com essa busca.'}
            </p>
          ) : (
            <div className="space-y-1.5">
              {shown.map(p => (
                ...
                >
                  ...
                  <span className="shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600">
